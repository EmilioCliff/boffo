...
// versions:
//   sqlc v1.30.0
// source: reseller_page-helpers.sql
...
const getResellerDashboardData = `-- name: GetResellerDashboardData :one
...
func (q *Queries) GetResellerDashboardData(ctx context.Context, resellerID int64) ([]byte, error) {
	row := q.db.QueryRow(ctx, getResellerDashboardData, resellerID)
	var dashboard_data []byte
	err := row.Scan(&dashboard_data)
	return dashboard_data, err
}

const getResellerGoodsRequestsPageStats = `-- name: GetResellerGoodsRequestsPageStats :one
SELECT 
  json_build_object(
...
FROM goods_requests
WHERE reseller_id = $1 AND cancelled = false
`

func (q *Queries) GetResellerGoodsRequestsPageStats(ctx context.Context, resellerID int64) ([]byte, error) {
	row := q.db.QueryRow(ctx, getResellerGoodsRequestsPageStats, resellerID)
...
func (q *Queries) GetResellerSalesPageStats(ctx context.Context, resellerID int64) ([]byte, error) {
	row := q.db.QueryRow(ctx, getResellerSalesPageStats, resellerID)
	var sales_stats []byte
	err := row.Scan(&sales_stats)
	return sales_stats, err
}

const getResellerStockPageStats = `-- name: GetResellerStockPageStats :one
WITH total_units AS (
  SELECT COALESCE(SUM(rs.quantity), 0)::bigint AS total_units
  FROM reseller_stock rs
  WHERE rs.reseller_id = $1
...
func (q *Queries) GetResellerStockPageStats(ctx context.Context, resellerID int64) ([]byte, error) {
	row := q.db.QueryRow(ctx, getResellerStockPageStats, resellerID)
	var stock_stats []byte
	err := row.Scan(&stock_stats)
	return stock_stats, err
}
